...
// //   const lastUserDoc = useRef<any>(null);
// //   const lastMsgDoc = useRef<any>(null);
...
// //           setDoc(doc(db, "users", user.uid), { online: false }, { merge: true })
// //         );
...
// //   // ✅ Load users list with infinite scroll
// //   const fetchUsers = async () => {
// //     if (!currentUser) return;
// //     const q = query(collection(db, "users"), orderBy("name"), limitFn(10));
// //     const snap = await getDocs(q);
// //     lastUserDoc.current = snap.docs[snap.docs.length - 1];
// //     const arr: UserData[] = [];
...
// //     if (
// //       usersRef.current &&
// //       usersRef.current.scrollTop + usersRef.current.clientHeight >=
// //         usersRef.current.scrollHeight - 20
// //     ) {
// //       const q = query(
// //         collection(db, "users"),
// //         orderBy("name"),
// //         startAfter(lastUserDoc.current),
// //         limitFn(10)
// //       );
// //       const snap = await getDocs(q);
// //       lastUserDoc.current = snap.docs[snap.docs.length - 1];
...
// //         const docSnap = await getDoc(doc(db, "chats", chatId));
// //         const unread = docSnap.exists() ? docSnap.data()[`unread_${currentUser.uid}`] || 0 : 0;
// //         newUsers.push({ ...u, unreadCount: unread });
// //       }
// //       setUsers((prev) => [...prev, ...newUsers]);
// //     }
// //   };

// //   // ✅ Real-time message listener with infinite scroll
// //   useEffect(() => {
...
// //         <List>
// //           {users
// //             .filter((u) => u.name?.toLowerCase().includes(searchInput.toLowerCase()))
// //             .map((u) => (
// //               <ListItemButton key={u.uid} onClick={() => setSelectedUser(u)}>
// //                 <Badge
// //                   overlap="circular"
// //                   variant={u.online ? "dot" : undefined}
// //                   color="success"
// //                 >
...
// //                   <Typography fontSize={10} color="gray" textAlign="right">
// //                     {time}
// //                   </Typography>
// //                 </Box>
...
//       // ✅ Seen Logic
//       msgs.forEach((msg) => {
//         if (msg.senderId !== currentUser.uid && !msg.seenByReceiver) {
...
//             style={{ marginLeft: "10px", padding: "6px 12px", borderRadius: "4px", background: "#1976d2", color: "white", border: "none", height: "40px" }}
//           >Logout</button>
//         </Box>
...
//                   <Typography fontSize={14}>{m.text}</Typography>
//                   <Typography fontSize={10} color="gray" textAlign="right">{time} {isMe && m.seenByReceiver && "✓✓"}</Typography>
//                 </Box>
//                 {isMe && <Avatar src={avatarUrl} sx={{ width: 24, height: 24, ml: 1 }} />}
//               </Box>
//             );
